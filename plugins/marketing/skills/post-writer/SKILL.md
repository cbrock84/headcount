---
name: post-writer
description: Writes, formats, and scores social posts end to end — drafting in a defined voice, shaping them for how each platform actually renders text, and grading them against engagement benchmarks before they ship. Use this to draft a post, rewrite a weak one, format a draft for LinkedIn or X, or judge whether a post is worth publishing. Covers the full pipeline from blank page to publish decision.
---

# Post writer

Three stages, one skill: **draft → format → score**. They were separate once and collided on every
"write me a post" — they are steps in one pipeline, not alternatives.

## Pipeline

1. **Draft** — the sections below. Voice first, structure second.
2. **Format** — apply `references/post-formatter.md`. Line breaks, spacing, and where the fold
   falls change performance as much as the words do.
3. **Score** — apply `references/post-scorer.md` before publishing. A post that scores badly gets
   rewritten, not shipped with a caveat.

Do not skip scoring because the draft feels good. That is exactly when it is worth checking.
