---
name: voice-builder
description: Captures how someone actually writes and turns it into reusable voice instructions every other content skill draws on — analyzing existing samples where they exist, or building a voice from archetypes where they do not. Use this to define or refine a writing voice, onboard a new writer or client, set up a content system before drafting anything, or when output keeps coming back sounding generic. Extends to newsletter-specific voice once the base voice exists.
---

# Voice builder

The upstream dependency for every content skill. Draft anything before the voice is defined and it
comes back sounding like everyone else.

Produces `voice.md` and `about-me.md`, which the writing skills read. Once those exist,
`references/newsletter-voice.md` extends them into newsletter-specific instructions — it needs the
base voice first and will say so if it is missing.
